...
import { BASE_PATH_APP, BASE_PATH_WEB, PARAM_KEY, PARAM_IV, CREDENTIAL_PUBKEY, HEADER_X_AUTH_TOKEN, HEADER_CUST_NUMBER, JSON_KEY_STA, JSON_KEY_MESSAGE, JSON_KEY_DATA, JSON_KEY_CUST_NUMBER, AREACODE_FALLBACK, VERIFICATION_CODE_TYPE_LOGIN, SEND_MSG_TYPE_VERIFICATION_CODE, RESP_STA_SUCCESS, RESP_STA_QR_NOT_SCANNED, JSON_KEY_AREA_CODE, JSON_KEY_ACCT_ID, JSON_KEY_LOGON_CHAN, LOGON_CHANNEL_HANDHELD_HALL, JSON_KEY_CRED_TYPE, LOGIN_TYPE_PHONE_CODE, JSON_KEY_SMS_CODE, JSON_KEY_PARAM, LOGIN_TYPE_PHONE_PWD_CODE, JSON_KEY_ELE_CUST_ID, JSON_KEY_METERING_POINT_ID, JSON_KEY_METERING_POINT_NUMBER, JSON_KEY_YEAR_MONTH, QRCodeType, } from "./consts.js";
export { QRCodeType };
// 加密/解密辅助函数
/**
 * 使用 RSA 公钥加密用户密码
 */
export function encryptCredential(password) {
...
export function encryptParams(params) {
    ...
    const cipher = crypto.createCipheriv("aes-128-cbc", Buffer.from(PARAM_KEY, "utf8"), Buffer.from(PARAM_IV, "utf8"));
 ...
 */
export function decryptParams(encryptedStr) {
    const decipher = crypto.createDecipheriv("aes-128-cbc", Buffer.from(PARAM_KEY, "utf8"), Buffer.from(PARAM_IV, "utf8"));
...
export class CSGClient {
     ...
     * 发起 HTTP 请求
     */
    async makeRequest(path, payload, options = {}) {
        const { withAuth = true, method = "POST", customHeaders = {}, basePath = BASE_PATH_APP, } = options;
     ...
     * 处理不成功的 API 响应
     */
    handleUnsuccessfulResponse(path, responseData) {
    ...
    async getMonthDailyCostDetail(account, year, month) {
        try {
            const respData = await this.apiQueryDayElectricChargeByMPoint(year, month, account.areaCode, account.eleCustomerId, account.meteringPointId);
            const byDay = (respData.result || []).map((dData) => ({
                date: dData.date,
                charge: parseFloat(dData.charge),
                kwh: parseFloat(dData.power),
            }));
            const monthTotalCost = respData.totalElectricity !== null
            ...
            const currentLadderRemainingKwh = respData.ladderEleSurplus !== null
                ? parseFloat(respData.ladderEleSurplus)
                : null;
            const currentTariff = respData.ladderEleTariff !== null
            ...
            try {
                ...
                const matchedItem = (feeDetails.electricAndChargeList || []).find((item) => item.yearMonth === targetMonthStr);
                if (matchedItem) {
            ...
            try {
                const dayElectric = await this.apiQueryDayElectricByMPoint(year, month, account.areaCode, account.eleCustomerId, account.meteringPointId);
