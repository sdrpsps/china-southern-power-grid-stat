...
**Reason**: The Skill package no longer includes a local command-line runtime, so it has no responsibility for locating session files.
**Migration**: Session lookup remains part of the MCP service and related login/profile behavior. Users configure credentials where the MCP service runs.

### Requirement: Skill 命令输出契约
**Reason**: The Skill package no longer executes a command that writes JSON to stdout or stderr.
**Migration**: Structured data is returned by MCP tools. The Skill guide specifies how assistants should interpret MCP results and render user-facing answers.
